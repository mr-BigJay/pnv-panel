...
if(!function_exists('supportLoad')){
    ...
    function supportDiscoverBackupFiles($file){
    ...
    function supportLoad($file){
        ...
        if(count($merged) > $currentCount || !is_array($current)){
            supportSave($file, $merged, ['force' => true]);
        }
    ...
    function supportMessageForApi($message, $options = []){
        ...
        return [
            ...
            'avatar_initial' => $avatarMeta['initial']
        ];
    ...
    function supportTicketLastUserMessage($ticket){
        ...
        for($i = count($ticket['messages']) - 1; $i >= 0; $i--){
            ...
            if(supportNormalizeSender($msg['sender'] ?? '') === 'user'){
                return $msg;
            }
        }
    ...
    function supportTicketPreviewMessage($ticket){
        ...
        for($i = count($ticket['messages']) - 1; $i >= 0; $i--){
            ...
            if(!is_array($msg) || supportNormalizeSender($msg['sender'] ?? '') !== 'user'){
                continue;
            }

            if(supportExtractMessageText($msg) !== '' || trim((string)($msg['image'] ?? '')) !== ''){
                return $msg;
            }
        }
    ...
    function supportTicketPreview($ticket){
        ...
        if($text === '' && trim((string)($last['image'] ?? '')) !== ''){
            return '📷 تصویر';
        }
    ...
    function supportMessageAvatarMeta($sender, $options = []){

        require_once __DIR__ . '/profile_lib.php';

        $chatUser = trim((string)($options['chatUser'] ?? ''));

        if(($sender ?? '') === 'admin'){
            return [
                'avatar' => profileGetSupportAdminAvatar(),
                'initial' => 'پ',
                'label' => 'پشتیبانی'
            ];
        }
        ...
        return [
            'avatar' => profileGetUserAvatar($chatUser),
            'initial' => supportUserInitial($chatUser),
            'label' => $chatUser !== '' ? $chatUser : 'کاربر'
        ];

    }

    function supportRenderAvatarHtml($sender, $options = []){

        $meta = supportMessageAvatarMeta($sender, $options);
        $avatar = $meta['avatar'];
        $initial = htmlspecialchars($meta['initial'], ENT_QUOTES, 'UTF-8');
        $label = htmlspecialchars($meta['label'], ENT_QUOTES, 'UTF-8');

        if($avatar !== ''){
            $src = htmlspecialchars('/' . ltrim($avatar, '/'), ENT_QUOTES, 'UTF-8');

            return '<div class="msgRowAvatar" title="' . $label . '"><img src="' . $src . '" alt=""></div>';
        }

        return '<div class="msgRowAvatar" title="' . $label . '"><span class="msgRowInitial">' . $initial . '</span></div>';

    }

    function supportRenderConvAvatarHtml($username){
    ...
    function supportRenderAdminConvAvatarHtml(){
